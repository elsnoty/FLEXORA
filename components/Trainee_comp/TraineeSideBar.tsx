...
import {
    ...
    const renderLinks = () => (
        ...
        {links.map((link) => (
            ...
            className="flex items-center gap-3 text-sm font-medium hover:text-primary transition-colors duration-200"
            >
            {link.icon}
            {link.label}
            </Link>
    ...
    return (
        ...
        <div className="md:hidden absolute top-4 left-4 z-50">
            ...
            <SheetTrigger asChild>
                <button className="p-2 fixed top-4 left-4 rounded-md hover:bg-accent transition-colors duration-200">
                <Menu className="h-6 w-6" />
                </button>
            </SheetTrigger>
            <SheetContent side="left" className="w-64 flex flex-col justify-between bg-background">
                <div>
                <SheetHeader>
                    <Link href="/">
                    <SheetTitle>
