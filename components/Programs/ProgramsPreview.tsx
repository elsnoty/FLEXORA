...
import { Dumbbell, Calendar, BarChart, Plus, ShoppingCart } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { CreateProgramForm } from "@/app/trainer/programs/create-program-form";

export default function ProgramsPreview({programs}: {programs: Program[]}) {
      ...
      {programs.length === 0 ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {programs.map((program) => (
            <ProgramCard key={program.id} program={program} isTrainer={false} />
          ))}
        </div>
      )}
    </div>
...
export function ProgramCard({ program, isTrainer }: { program: Program, isTrainer?: boolean }) {
